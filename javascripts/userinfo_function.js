...
window.onload = function () {
    ...
    quetionPath.once('value').then(function (snapshot) {
        snapshot.forEach(function (userSnapshot) {
    ...
    firebase.database().ref('/users/' + result_search_id + '/quetion/3/').once('value').then(function (snapshot) {
        ...
        //임시로 생성해둠.
        for (let i = 0; i < userQuetionVal.length; i++) { //질문들이 담겨져있는 userQuetionVal배열만큼 반복해준다.
            var dd = userQuetionVal[i];
            innerQuetion += '<div><p>' + dd + '</p></div>';
        }
